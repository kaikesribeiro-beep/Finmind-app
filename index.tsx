...
import { LayoutDashboard, Receipt, PieChart, Plus, CheckCircle2, Wallet, Target, Calculator } from 'lucide-react';
import './index.css';

// --- COMPONENTES INTERNOS (Para não dar erro de arquivo faltando) ---
...
const SimulationPanel: React.FC<{ transactions: Transaction[] }> = ({ transactions }) => {
  ...
  return (
    <div className="bg-neutral-900/50 p-4 rounded-xl border border-neutral-800 space-y-4">
      <h3 className="text-lg font-bold text-white flex items-center gap-2"><Calculator size={18} className="text-blue-500"/> Simulador de Metas</h3>
      <div className="space-y-3">
        <div>
            <label className="text-xs text-neutral-400">Eu quero juntar (R$):</label>
        ...
        <div className="bg-blue-500/10 p-3 rounded-lg border border-blue-500/20 text-center">
            <p className="text-xs text-blue-300 mb-1">Com sua economia atual de {savings.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}/mês:</p>
            <p className="text-xl font-bold text-white">{calculate()} meses</p>
            <p className="text-[10px] text-neutral-500">para atingir seu objetivo</p>
        </div>
      </div>
    </div>
  );
...
const App = () => {
  ...
  // FUNÇÕES NOVAS (Backup e Limpeza)
  ...
  return (
    <div className="min-h-screen pb-24 md:pb-8 bg-neutral-950 text-neutral-50 selection:bg-emerald-500 selection:text-white">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {/* VIEW: TRANSACTIONS (Aqui entram seus filtros novos!) */}
        ...
        {/* VIEW: PLANNING (Aqui está o Planejamento recuperado!) */}
        {activeTab === 'planning' && (
